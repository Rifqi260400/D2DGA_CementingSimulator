...
|---|---|
| **Mud τ̂_Y, κ̂, n** (shown as 4.79 Pa, 0.020, 0.70) | **Available since 2026-09-19** as `mud_yield_stress`, `mud_consistency`, `mud_power_law_index` (G-1, approved). The mockup's own values are outside the validated pair, so a setup screen offering them must carry the B-1 disclaimer — see FLU-07. |
...
Ordered by how much they block the build. **G-1, G-2 and G-3 are resolved**
(G-1 and G-2 done; G-3 needed no solver change). The open items are G-4..G-8 and
what remains of G-9, all of which belong to the screens still to be built.

| ID | Gap | Proposal |
|---|---|---|
| **G-1** | ~~`FluidsConfig` cannot express a non-Newtonian mud.~~ **DONE 2026-09-19, approved by the user — and measuring it corrected a claim in the audit.** | `mud_consistency`, `mud_power_law_index`, `mud_yield_stress` added; `mud_viscosity` is now a property that refuses for a non-Newtonian mud rather than returning κ̂ under a name that means Pa·s. Both fluids validated at construction (`n > 1` refused, not extrapolated). Defaults bit-identical including `repr`, which the closure-table cache key depends on. **The correction:** B-1 was held to be benign on K-GEP-1 "because the displaced fluid is Newtonian". It is not. Holding the mud strictly Newtonian and moving only its viscosity gives B-1 = `0.53%` at 1 mPa·s, `11.4%` at 5 mPa·s, `93.1%` at 20 mPa·s; a fully Newtonian pair gives exactly `0.00%`. The governing quantity is `m`, not the mud's rheology — so **this hazard predates G-1 and reverting G-1 would not close it**. The disclaimer is therefore keyed to the measured value: the Results screen shows the run's own `assumption_report()` from `metrics.json`, names every fluid field that differs from the validated pair, and shows a warning rather than a green tick when no measurement was recorded. REM-11 / FLU-07; 11 gates in `tests/test_m0_config.py`. |
...
| **G-3** | ~~Live signals cannot leave the process.~~ **DONE 2026-09-19.** | `--status-json PATH` makes the runner rewrite a small JSON document atomically (temp + `os.replace`) every ~1.5 s from its existing `on_step` hook, plus a `preparing` stub before stepping starts — the closure-table build is often the longest part and a monitor that shows nothing there cannot be told from a crash. One thing the solver did not emit had to be added: the running volume error existed only as a local in `run`, so a monitor would have had to recompute an invariant. `Simulation.conservation_error` is now set where `cons_err` is updated (REM-12). The runner stays a plain subprocess, so a UI run and a CLI run are the same invocation — asserted by UI-2, which parses the launcher's argv back with `build_parser()`. |
| **G-4** | ~~No η_N scalar.~~ **DONE 2026-09-19.** | `postprocess.narrow_side_efficiency(geometry, c, fraction=0.25)` — the same volume-weighted integral as η_E over the narrowest quarter of φ. It belongs in the solver, not the UI (spec §5), and the runner prints it and records it in `metrics.json`. On K-GEP-1: η_E 0.9946, η_N 0.9939, worst column 0.9494. Gated by M7-T7, which pins it to η_E at `fraction = 1` and to zero on a wide-side-only field. |
| **G-5** | ~~The mockup's ZF23 "regime map" does not exist.~~ **DONE 2026-09-19.** | `tests/benchmarks/bf25_muskat.py` moved unchanged to `d2dga/muskat.py`, because a screen must not import from `tests/` and this is the only a-priori prediction the codebase can make from setup inputs alone. The Case-setup screen computes Δw(0⁺) from this case's own closures; on the shipped pair it returns **−10.57, "stable", finger absorbed**, agreeing with the committed gate `test_bench10_...` (`dw < -10`). ⚠️ The probe table it builds has a single velocity node at |ū| = 1 (BF25 §3.1's own evaluation point), so its B-1 figure is *not* the run's — the screen says so explicitly. |
| **G-6** | ~~No pause.~~ **DONE 2026-09-19.** | There is no Pause button and the monitor says why: the solver has no paused state and a button claiming otherwise would misdescribe what it does. **Stop** sends SIGTERM; the run checkpoints every 180 s, so the same command resumes it, and the page says that too. |
| **G-7** | ~~The Validation screen has no machine-readable source.~~ **DONE 2026-09-19.** | `tests/run_gates.py` — a pytest *plugin*, so no test is modified, skipped or reordered — writes `output/gates.json` with per-module counts, failures and the git SHA. The screen reads it and **refuses to fall back** to the prose in `docs/gate_status.md`, because a hand-written file cannot go stale loudly. It also warns when the tree was dirty or the results are over a day old. |
| **G-8** | ~~The mockup's Validation numbers are wrong.~~ **DONE 2026-09-19, and the first implementation reproduced the mockup's error by accident.** | The screen parses `output/zf22_table3.md` and shows **9 / 10**, case 1 in rust at −44.5 %, linked to BLK-6. The first parser filtered rows on `startswith("\| ")` and then sliced `[2:]`; the file writes its separator as `\|---\|---\|` with no space, so the filter removed the separator and the slice removed **case 1** — the one case that fails. The screen displayed **9 / 9**: a clean bill of health manufactured by discarding the counter-evidence, invisible in the code and caught only by looking at the rendered page. Parsing now lives in `ui/reports.py` and is gated by UI-12/13/14, which assert all ten rows survive, that the separator is found by content, and that an unreadable row counts as *not* reproduced. |
| **G-9** | ~~`Config` has no field for CFL, inflow, wall, volumes, or the fluids.~~ **DONE 2026-09-19.** | `build_parser()` was extracted first (the UI introspects it for names, defaults, `choices` and help; `PARAM_META` adds units and ranges). The remainder — the fluids and the standoff living only in `config.py`, so a UI run on anything but the defaults could not be reproduced from a command line — is closed by `--mud-*`, `--cement-*` and `--eccentricity`, whose defaults are *read from* the dataclasses. Checkpoints of different physics used to collide on one filename; a `_f<hash>` suffix separates them while the default keeps its historical name (UI-4). |
| **G-10** | ~~Re is reported but unused.~~ **DONE 2026-09-19.** | Shown greyed on Case setup as "not used", with the reason: BF25 (2.16) is a constraint with no inertia term. Its absence is informative, which is why it is displayed rather than dropped. |

## D-bis. What building the screens changed in the solver

Four things, all additive, none of them a change to the numerics. Recorded in
`docs/assumptions.md` as REM-12..15 with their gates.

| What | Why it could not live in the UI |
|---|---|
| `--status-json`, and a `preparing` stub before stepping | The monitor needs progress out of the process. Written from the existing `on_step`; the stub exists because the closure-table build is often the longest part, and a monitor showing nothing there cannot be told from a crash. |
| `Simulation.conservation_error` | The running volume error was a local in `run`. A monitor would have had to recompute it — **a second copy of an invariant**, which can disagree with the report about the same run. |
| `RunResult.steps` | Callers read `reports[-1].n`: short by up to `record_every − 1` on every run, and `IndexError` when a relaunch finds the job already finished. A launcher does that routinely. |
| `postprocess.narrow_side_efficiency`, `postprocess.tbr_relative_error` | η_N is a solver quantity (spec §5), and the t_br error constants were written down twice — once in the runner, once in the UI. |

Two more moved rather than appeared: `bf25_muskat.py` from `tests/` into
`d2dga/muskat.py` (a screen must not import from the test tree), and the
