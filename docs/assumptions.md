...
| BENCH-01 | PF04 (38) | **Re-derived, not transcribed** | **Source unreadable.** The display equations in the supplied PF04 PDF do not extract (text layer returns scrambled fragments) and no page renderer is available in this environment. | Derived from the model equations instead: pressure continuity across a sharp interface with fully-developed 1-D flow on each side. Matches the surviving fragments exactly once PF04's jump notation `[·]²₁` is recognised, including the sign of `b cos β`. Identification `1/𝓘₁,k ↔ χ_k(1) + τ_k,Y` — both are the modified pressure gradient driving unit areal flow through a unit half-gap. | **Yes — M4-T2 at β = 0, π/4, π/2** |
| BENCH-02 | PF04 (40)–(43) | **NOT reproduced — outstanding** | **Source unreadable**, as BENCH-01. | Their coefficients `P(χ,τ_Y,m)`, `α_k`, `tanh α_k L` cannot be recovered from the PDF, and deriving the full O(e) perturbation solution is out of scope. M4-T3 therefore asserts only the verifiable part: at small `e` the solution stays near the concentric steady state and approaches it as `e → 0`. **M4-T3 is PARTIAL.** To close it, the equations must be read from a rendered copy of PF04 pp. 570–571. | Partial |
